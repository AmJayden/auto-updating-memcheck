...
utils::segment utils::text_seg = utils::segment{ ".text" }, utils::vmp0_seg = utils::segment{ ".vmp0" };
...
{
    return
        !std::memcmp(reinterpret_cast<void*>(address), "\x55\x8B\xEC", 3) ||
        !std::memcmp(reinterpret_cast<void*>(address), "\x53\x8B\xDC", 3) ||
        !std::memcmp(reinterpret_cast<void*>(address), "\x56\x8B\xF4", 3);
};
